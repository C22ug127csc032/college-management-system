...
const NAV = [
  ...
  { to: '/admin/circulars', label: 'Circulars', icon: FiBell },
  { to: '/admin/library', label: 'Library', icon: FiBookOpen },
  { to: '/admin/shop', label: 'Shop & Canteen', icon: FiShoppingBag },
  { section: 'Settings' },
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-primary-700">
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleLogout} className="text-primary-300 hover:text-white transition-colors text-lg" title="Logout">
            <FiLogOut />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 shrink-0">
          <button onClick={() => { setSidebarOpen(!sidebarOpen); setMobileOpen(!mobileOpen); }}
            className="p-2 rounded-lg hover:bg-gray-100 text-gray-600 transition-colors"><FiMenu /></button>
