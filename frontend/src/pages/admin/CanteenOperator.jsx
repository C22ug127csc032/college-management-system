...
export default function CanteenOperator() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const r = await api.get('/shop/items?type=canteen');
  ...
  const fetchSales = useCallback(async () => {
    try {
      const r = await api.get(`/shop/sales?type=canteen&date=${saleDate}`);
  ...
  const handleBill = async () => {
    ...
    if (paymentMode === 'wallet' && cartTotal > studentData.wallet.balance) {
      toast.error(
        `Insufficient wallet balance. Available: ₹${studentData.wallet.balance}`
      );
      return;
    }
  ...
  const handleAddItem = async e => {
    e.preventDefault();
    try {
      await api.post('/shop/items', { ...itemForm, type: 'canteen' });
      toast.success('Item added');
      setShowAddItem(false);
      setItemForm({ name: '', price: '', unit: 'portion', stock: '', minStockAlert: 5 });
      fetchItems();
    } catch (err) {
      ...
      {tab === 'billing' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="card">
              ...
              {studentData && (
                <div className="flex items-center justify-between mt-3
                  p-3 bg-orange-50 border border-orange-200 rounded-xl">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-orange-200
                      text-orange-800 font-bold flex items-center
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 font-mono">
                        {studentData.student.rollNo || studentData.student.admissionNo}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
            ...
            {studentData && (
              <div className="card">
                <h3 className="section-title mb-3">Menu</h3>
                {menuItems.length === 0 ? (
                  <p className="text-center py-6 text-gray-400 text-sm">
                    No items available. Add items in Menu Items tab.
                  </p>
                ) : (
                        ...
                        <button
                          ...
                          {inCart && (
                            <span className="absolute top-2 right-2
                              w-5 h-5 bg-orange-500 text-white text-xs
              ...
              <h3 className="section-title mb-3 flex items-center gap-2">
                ...
                {cart.length > 0 && (
                  <span className="ml-auto text-xs text-gray-400
                    font-normal cursor-pointer hover:text-red-500"
                    onClick={() => setCart([])}>
                    Clear
                  </span>
                )}
      ...
      {tab === 'items' && (
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="section-title">Canteen Menu Items</h3>
            <button
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-6 h-6 rounded bg-gray-100 hover:bg-red-100
                            text-gray-600 flex items-center justify-center text-xs">
                          <FiMinus />
                        </button>
          ...
          {todaySummary && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                {
                  label: 'Today Revenue',
                  value: `₹${todaySummary.totalRevenue?.toLocaleString('en-IN') || 0}`,
                  color: 'bg-orange-50 text-orange-700',
                },
                {
                  label: 'Wallet Sales',
                  value: `₹${todaySummary.walletRevenue?.toLocaleString('en-IN') || 0}`,
                  color: 'bg-blue-50 text-blue-700',
                },
                {
                  label: 'Cash Sales',
                  value: `₹${todaySummary.cashRevenue?.toLocaleString('en-IN') || 0}`,
                ...
                <tbody className="divide-y divide-gray-50">
                  {sales.map(sale => (
                    <tr key={sale._id} className="hover:bg-gray-50">
                      ...
                      <td className="table-cell font-bold text-orange-600">
                        ₹{sale.totalAmount}
                      </td>
                      <td className="table-cell">
